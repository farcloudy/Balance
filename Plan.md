...
- ✅ 前端 Axios 请求封装（token 自动添加、401 拦截）
- ✅ 前端代码规范配置（Prettier、ESLint）
- ✅ 前端环境变量和开发代理配置
...
- ✅ 多租户数据隔离
- ✅ Entry 模型定义
- ✅ 分录 CRUD API（创建、查询、修改、过账、冲销）
- ✅ 凭证号生成工具
- ✅ 凭证验证工具（借贷平衡、科目有效性、日期合理性）
- ✅ 金额格式化工具
- ✅ 分录常量定义（状态、类型）

### 当前阶段
✅ 第四阶段（分录和凭证管理）已完成，准备进入第五阶段（AI 辅助录入）。
...
- ✅ 创建 `routes/auth.js`
  ...
  - **注意**：后端只做必填字段检查和业务逻辑验证（如用户名是否已存在）
...
- ✅ 实现注册/登录表单验证
  - 用户名长度：3-20 个字符
  - 用户名格式：只允许字母、数字和下划线（正则：`/^[a-zA-Z0-9_]+$/`）
  - 密码长度：至少 8 个字符
  - 密码强度：建议包含大小写字母、数字、特殊字符
  - 即时验证反馈，提升用户体验
- ✅ 实现密码加密
  - 使用 `crypto-js` 对密码进行 SHA-256 加密
  - 在发送到后端前完成加密
  - 后端会对加密后的字符串再次进行 bcrypt 哈希存储
...
### 安全要求
- 前端密码加密 + 后端哈希存储
- JWT token 有效期设置合理（默认 7 天）
- 本项目运行在 Windows 本地内网环境

---

## 开发顺序建议

1. **第一阶段**（后端已完成，前端待完善）→ **第二阶段** → **第三阶段** → **第四阶段**（核心功能）
2. **第五阶段**（特色功能，可并行开发）
3. **第六阶段**（报表功能）
4. **第七阶段**（优化完善）
